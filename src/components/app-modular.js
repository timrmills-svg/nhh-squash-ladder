...
const SquashLadderApp = () => {
  ...
  const handleCreateChallenge = async (challengerId, challengedId) => {
    try {
      ...
      const challengedEmail = await UserLookupService.getEmailFromDisplayName(challenged.name);
      
      if (challengedEmail) {
        await EmailService.sendChallengeCreatedEmail(
          challenger.name,
          challengedEmail,
          challenged.name,
          window.location.origin
        );
        console.log(`Challenge email sent to ${challengedEmail}`);
      } else {
        console.warn(`Could not find email for player: ${challenged.name}`);
      }
