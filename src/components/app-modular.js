...
import LoginWithGuide from './LoginWithGuide';
...
const SquashLadderApp = () => {
  ...
  useEffect(() => {
    console.log("Debug: showVideo changed to:", showVideo);
  }, [showVideo]);
  ...
  const checkExpiredChallenges = () => {
    ...
    const expiredChallenges = challenges.filter(challenge => {
      if (challenge.status === "accepted" && challenge.expiryDate) {
        return new Date(challenge.expiryDate) < now;
      }
      return false;
    });
  ...
  const stats = {
    ...
    activeChallenges: challenges.filter(c => c.status === "accepted").length,
    matchesPlayed: matches.length,
    currentLeader: players.find(p => p.position === 1)?.name || "No leader"
  };
      ...
      case "rules":
        return (
          <div className="bg-white rounded-lg shadow p-6">
            <h2 className="text-2xl font-bold mb-4">Ladder Rules</h2>
            <div className="prose max-w-none">
              <p>Official LSRC squash ladder rules apply.</p>
            </div>
          </div>
        );
      default:
  ...
  useEffect(() => {
    if (!currentUser) {
    console.log("Debug: User not logged in, showVideo:", showVideo, "currentUser:", currentUser);
